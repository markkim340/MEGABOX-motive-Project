...
1. 상영스케쥴 테이블의 생성
  ...
  - 영화 예매 방법에 대한 고민.
    영화/상영일자/영화관/상영관/상영시간을 선택.
    본 프로젝트에서는 전국에 108개 지점의 STARBOX 영화관이 있고 각 지점별 영화관마다 3개의 상영관이 있다고 가정하였다. 상영관에서는 하루에 5번의 스케쥴로 특정 영화가 상영된다. 각각의 스케쥴별로 컬럼을 생성하면 하루에 영화관의 수(108) x 상영관의 수(3) x 상영횟수(5) = 1620개의 데이터가 생성된다. 이것은 하루동안의 전국 상영스케쥴이고 상영일자를 추가하기 위해 프로젝트의 제작기간에 맞추어 22.09.30부터 22.10.07 까지 7일간의 상영스케쥴을 만들었다. 데이터는 1620 x 7 = 11,340개가 된다. (데이터 생성은 구글 스프레드시트 데이터 자동완성 기능을 통해 상영일자별 상영시간표를 다 채운뒤 movie_id가 1~8까지 랜덤으로 부여되게 생성하였으며, 만들어진 데이터를 csv파일로 전환하여 MySQL에서 import하였습니다.)
    
2. 상영관별 좌석데이터 구조에 대한 고민
  
  - 예약자는 상영스케쥴을 선택 한 뒤 좌석을 선택해야한다. 좌석은 각각의 상영관별로 구조/갯수가 동일하지만, 상영스케쥴별로 예약된 좌석정보와 잔여좌석 수가 다르다. 그렇다면 좌석의 구조(좌석번호)를 어떤식으로 데이터에 저장해야할까? Seats라는 테이블을 만든 뒤 다음과 같은 방법을 시도해보았다.
    
    - 상영스케쥴별로 좌석을 생성
      11,340개의 상영스케쥴별로 각각의 상영관별 좌석번호를 생성한다. 이 방법은 좌석이 100개일 경우 11,340 x 100 113,400개의 데이터가 생기고 데이터 만드는 것부터 시작해서 조회 및 예약까지 너무 처리가 복잡해지고 비효율적이었다. 그래서 다음과 같은 방법을 고안했다.
      
    - 상영관별로 고정된 좌석번호를 생성
      상영스케쥴 정보를 전달해줄때 해당 상영스케쥴의 id값과 = Ticket 테이블의 스케쥴id가 같은 값을 조회하여 예약된 좌석정보를 함께 전달해준다. 이 방법이 훨씬 효율적이고 생성되는 컬럼도 적어진다.
      
    
    그렇다면 좌석구조는 어떤식으로 저장하여야 할까? 또 다음과 같은 방법을 시도하였다.
  ...
  1. 2차원 배열을 통한 저장
    ...
    - [[A1,A2,...A9,A10],[B1,B2,...B9,B10],[..],[..],[..]...[...E9,E10]]
      
  
3. 상영스케쥴 필터 조회
  
4. 상영스케쥴별 좌석정보 전달과 티켓생성
...
1. 백엔드에서 유효성검사에 대한 처리가 미흡
  ...
  - 프론트에서 기본적인 유효성 검사가 진행되지만 백엔드에서도 예외처리가 진행되어야 할 부분임.
2. 동시에 좌석예매를 진행할 경우 서버에서 처리하는 방법에 대한 고민
  
  - 트랜젝션으로 처리
3. 부킹아이디 까지 생성 후 티켓생성에서 에러가 날 경우 롤백에 대한 고민
